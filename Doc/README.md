...
- [依赖项]：rust clippy、tree-sitter、rag代码库(所有依赖项已提前安装部署)

## **6.2**、安装步骤

- 进入源代码根目录 cd 000034-ZheLiYouGeHaoMingDui/

- 运行命令docker run -d --user root -v "$PWD":/usr/src/zheliyougehaomingdui -w /usr/src/zheliyougehaomingdui zheliyougehaomingdui tail -f /dev/null

- 运行命令docker ps 查看先前运行的\<container id\>
