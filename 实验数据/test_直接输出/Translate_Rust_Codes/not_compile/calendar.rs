...
fn print_row(row: i32, months: &[Month; 12], cols: i32, lead: i32, gap: i32) {
    let from = row * cols;
    let to = from + cols;
    ...
    loop {
        ...
        space(lead);
        for c in from..to {
            let month = &months[c as usize];
            for _ in 0..month.start_wday {
                space(3);
            }
            let mut i = month.start_wday;
            while i < 7 && month.at < month.days {
                print!("{:2}", month.at + 1);
                month.at += 1;
                if i < 6 || c < to - 1 {
                    print!(" ");
...
fn print_year(year: i32, months: &[Month; 12], cols: i32, lead: i32, gap: i32) {
    let buf = format!("{}", year);
...
fn main() {
    ...
    let gap = if cols > 1 { (width - 20 * cols) / (cols - 1) } else { 0 };
    let lead = (width - (20 + gap) * cols + gap + 1) / 2;
    print_year(year, &months, cols, lead, gap);
}