...
impl Cidr {
    fn parse(str: &str) -> Option<Self> {
        ...
        let a: u8 = address_parts[0].parse().ok()?;
        let b: u8 = address_parts[1].parse().ok()?;
        let c: u8 = address_parts[2].parse().ok()?;
        let d: u8 = address_parts[3].parse().ok()?;
