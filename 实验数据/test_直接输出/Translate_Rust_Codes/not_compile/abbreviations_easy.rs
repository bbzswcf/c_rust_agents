...
fn make_command_list(table: &str) -> Option<Box<Command>> {
    ...
    for word in words {
        ...
        new_cmd.cmd = word.chars().map(|c| c.to_ascii_uppercase()).collect();
        new_cmd.next = cmd.take();
        cmd = Some(new_cmd);
    }
    cmd
}

fn free_command_list(mut cmd: Option<Box<Command>>) {
    while let Some(mut command) = cmd.take() {
        cmd = command.next.take();
    }
}
