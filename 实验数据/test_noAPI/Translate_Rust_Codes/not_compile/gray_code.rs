...
fn fmtbool(n: i32, buf: &mut [char; 6]) {
    ...
    b -= 1;
    let mut n = n;
    // Modified: Changed the loop condition to `b > 0` to avoid comparing `usize` with `usize::MAX`
    while b > 0 {
        // Modified: Cast the result of `n & 1` to `u8` before casting to `char`
        buf[b] = '0' + ((n & 1) as u8) as char;
        n >>= 1;
        if b == 0 {
            break;
        }
