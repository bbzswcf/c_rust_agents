...
fn create_deck(deck: &mut Box<Vec<i32>>, n_cards: usize) -> bool {
    *deck = Box::new(vec![0; n_cards]);
    deck.is_empty()
}
...
fn duplicate_deck(dest: &mut Box<Vec<i32>>, orig: &[i32], n_cards: usize) -> bool {
...
fn shuffle_deck(deck: &mut [i32], n_cards: usize) -> bool {
    let mut copy = Box::new(vec![0; n_cards]);
...
fn main() {
    let mut deck: Box<Vec<i32>> = Box::new(vec![]);

    for i in 0..N_DECKS {
        let n_cards = K_DECKS[i];
