...
fn chao(input: &str, output: &mut String, mode: i32, show_steps: bool) {
    let len = input.len();
    ...
    for (i, c) in input.chars().enumerate() {
        if show_steps {
            // Modified: Use the `collect` method which is more idiomatic in Rust
...
fn main() {
    ...
    chao(plain_text, &mut cipher_text, ENCRYPT, TRUE);
    println!("\nThe ciphertext is : {}", cipher_text);
    chao(&cipher_text, &mut plain_text2, DECRYPT, FALSE);
