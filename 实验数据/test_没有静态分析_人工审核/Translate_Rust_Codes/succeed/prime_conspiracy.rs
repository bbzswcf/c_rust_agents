...
// Modified: Added `Copy` and `Clone` traits to the `Transition` struct
#[derive(Copy, Clone)]
struct Transition {
    a: u8,
    b: u8,
...
// Modified: Used `LazyLock` to lazily initialize the static array
static TRANSITIONS: LazyLock<Mutex<[Transition; TRANSITIONS_SIZE]>> = LazyLock::new(|| {
    Mutex::new(core::array::from_fn(|_| Transition { a: 0, b: 0, c: 0 }))
});
...
fn init() {
    for i in 0..10 {
        for j in 0..10 {
            let idx = i * 10 + j;
            // Modified: Used `Mutex` to safely mutate the static item
...
fn print_transitions(limit: i32, last_prime: i32) {
    ...
    for i in 0..100 {
        // Modified: Used `Mutex` to safely access the static item
