...
fn main() {
    ...
    for i in (3..lim).step_by(2) {
        if i > 5 && i < lim - 6 && sv[i] == FALSE && sv[i - 6] == TRUE && sv[i + 6] == TRUE {
            un += 1;
            if un > unsexy - lun {
                let ix = (un + lun - 1).checked_sub(unsexy).unwrap_or(0); // Ensure safe subtraction
                last_un[ix] = i as i32;
            }
            continue;
        }
        if i < lim - 6 && sv[i] == FALSE && sv[i + 6] == FALSE {
            pr += 1;
            if pr > pairs - lpr {
                let ix = (pr + lpr - 1).checked_sub(pairs).unwrap_or(0); // Ensure safe subtraction
        ...
        if i < lim - 12 && sv[i + 12] == FALSE {
            tr += 1;
            if tr > trips - ltr {
                let ix = (tr + ltr - 1).checked_sub(trips).unwrap_or(0); // Ensure safe subtraction
        ...
        if i < lim - 18 && sv[i + 18] == FALSE {
            qd += 1;
            if qd > quads - lqd {
                let ix = (qd + lqd - 1).checked_sub(quads).unwrap_or(0); // Ensure safe subtraction
        ...
        if i < lim - 24 && sv[i + 24] == FALSE {
            qn += 1;
            if qn > quins - lqn {
                let ix = (qn + lqn - 1).checked_sub(quins).unwrap_or(0); // Ensure safe subtraction
