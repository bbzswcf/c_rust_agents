...
fn _parse_hex(pch_cursor: &mut &str) -> u32 {
    ...
    while let Some(ch_now) = pch_cursor.chars().next() {
        ...
        if (ch_now >= '0' && ch_now <= '9') || (ch_now >= 'A' && ch_now <= 'F') {
            let nybble_value = match ch_now {
                '0'..='9' => ch_now as u32 - '0' as u32,
                'A'..='F' => ch_now as u32 - 'A' as u32 + 10,
                _ => unreachable!(),
            };
            n_val <<= 4;
            n_val += nybble_value;
...
fn parse_ipv4_or_ipv6(ppsz_text: &mut &str, aby_addr: Option<&mut [u8]>, pn_port: Option<&mut u16>, pb_is_ipv6: Option<&mut bool>) -> bool {
    let mut aby_addr_local = aby_addr.map(|a| a.as_mut_ptr()).unwrap_or(ptr::null_mut());
    let aby_dummy_addr = &mut [0u8; 16];
    ...
    if aby_addr_local.is_null() {
        aby_addr_local = aby_dummy_addr.as_mut_ptr();
    }

    if !b_is_ipv6_local {
        let mut pby_addr_cursor = aby_addr_local;
        let mut n_val;
        ...
        *ppsz_text = &ppsz_text[1..];
        ...
        if ppsz_text.chars().next() == Some(':') && pn_port.is_some() {
            ...
            *ppsz_text = &ppsz_text[1..];
            ...
            us_port_network = (n_val >> 8) as u8 as u16 | ((n_val & 0xff) as u8 as u16) << 8;
        ...
        if !pby_zeros_loc.is_null() {
            ...
            unsafe {
                ptr::copy(pby_addr_cursor.sub(n_tail), pby_addr_cursor, n_tail);
        ...
        if b_ipv4_detected {
            ...
            if unsafe { slice::from_raw_parts(aby_addr_local, 12) != ABY_PFX } {
                return false;
            }
        }
...
fn parse_ipv4_or_ipv6_2(psz_text: &str, aby_addr: Option<&mut [u8]>, pn_port: Option<&mut u16>, pb_is_ipv6: Option<&mut bool>) -> bool {
