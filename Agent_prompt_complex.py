...
Format:
C: [C API function]
Rust: [Equivalent Rust function or method]

Focus on the following types of functions:
1. Standard input/output functions (e.g., printf, scanf, puts)
2. String manipulation functions (e.g., strcpy, strcat, strlen)
3. Memory management functions (e.g., malloc, free, realloc)
4. File operations (e.g., fopen, fclose, fread, fwrite)
5. Math library functions (e.g., sqrt, pow, sin, cos)
6. Time-related functions (e.g., time, ctime, strftime)
7. Networking functions (e.g., socket, connect, send, recv)
8. Multithreading functions (e.g., pthread_create, pthread_join)
9. Common third-party library functions (e.g., functions from OpenSSL, libcurl)
...
Example output:
C: malloc
Rust: Vec::with_capacity or Box::new
...
Provide only API conversions that appear in the given C code. 
Do not include functions that are not used in the code.
"""

# 语法专家Prompt
Syntax_prompt = """
Use the provided API mappings to convert C code to Rust code. 
...
C: union
Rust: union or enum with variants
