...
pub extern "C" fn list_free(mut list: *mut ListEntry) {
    ...
    while !entry.is_null() {
        let next: *mut ListEntry = unsafe { (*entry).next };
...
pub extern "C" fn list_prepend(
    ...
    let newentry: *mut ListEntry;
    unsafe {
        newentry = alloc_test_malloc(std::mem::size_of::<ListEntry>() as libc::c_ulong) as *mut ListEntry;
    }
...
pub extern "C" fn list_to_array(mut list: *mut ListEntry) -> *mut ListValue {
    ...
    unsafe {
        array = alloc_test_malloc(
            (std::mem::size_of::<ListValue>() as libc::c_ulong).wrapping_mul(length as libc::c_ulong),
        ) as *mut ListValue;
    }
    if array.is_null() {
        return std::ptr::null_mut();
...
pub extern "C" fn list_iter_has_more(mut iter: *mut ListIterator) -> libc::c_int {
    unsafe {
        if ((*iter).current).is_null() || (*iter).current != *(*iter).prev_next {
            return (*(*iter).prev_next != std::ptr::null_mut() as *mut ListEntry) as libc::c_int;
        } else {
            return ((*(*iter).current).next != std::ptr::null_mut() as *mut ListEntry) as libc::c_int;
        }
    }
}
#[no_mangle]
