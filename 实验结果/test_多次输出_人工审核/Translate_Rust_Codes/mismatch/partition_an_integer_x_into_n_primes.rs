...
impl BitArray {
    ...
    fn set(&mut self, index: u32, value: bool) {
        ...
        let word_index = (index >> 5) as usize;
        let bit = 1 << (index & 31);
        if value {
            self.array[word_index] |= bit;
        } else {
            self.array[word_index] &= !bit;
        }
    }
...
impl Sieve {
    fn new(limit: u32) -> Option<Self> {
        let mut not_prime = BitArray::new(limit + 1)?;
...
fn main() {
    ...
    let sieve = Sieve::new(LIMIT).expect("Out of memory");
