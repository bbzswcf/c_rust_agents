...
def setup_logging():
    ...
    # 创建日志文件名（使用时间戳）
    log_file = f'Output/logs/conversion_{time.strftime("%Y%m%d_%H%M%S")}.log'
...
def process_files():
    ...
    #translation_order = get_translation_order(dependencies) # 获取翻译顺序

    # logging.info(f"测试文件翻译顺序：{translation_order}")
    
    # 读取 translation_order.txt 中的文件列表
    translation_order = []
    if os.path.exists('tool/translation_order.txt'):
        with open('tool/translation_order.txt', 'r', encoding='utf-8') as f:
            translation_order = [line.strip() for line in f if line.strip()]
    
    # 如果没有指定文件，则使用默认顺序
    if not translation_order:
        translation_order = ['test\\test-compare-functions.c', 'test\\test-hash-functions.c', 'test\\test-arraylist.c', 
                           'test\\test-avl-tree.c', 'test\\test-binary-heap.c', 'test\\test-binomial-heap.c', 
                           'test\\test-bloom-filter.c', 'test\\test-hash-table.c', 'test\\test-list.c', 
                           'test\\test-queue.c', 'test\\test-set.c', 'test\\test-slist.c', 'test\\test-trie.c', 
                           'test\\test-sortedarray.c', 'test\\test-rb-tree.c', 'test\\test-cpp.cpp']
