...
def insight_mapping(func_name: str):
    insight_prompt = ""
    if "insert" in func_name:
...
def convert_c_funcs_to_rust(c_file: str, func_name: str, c_func_info: dict, rust_items: str, rust_code_file: str,
    ...
    if 'test' not in func_name:
        insight=insight_mapping(func_name)
    else:
        insight=''
...
def convert_c_initialization_to_rust(c_file: str, rust_code_file: str, rust_result_dir: str, metadata: dict) -> str:
    ...
    logging.info("开始语法转换")
    if 'test' in c_file:
        combined_syntax_input = f"""Translate the following C definitions of types, structs, variables and macros to Rust. Initialize variables to default values.\nC code:\n{pre_code}\nOutput only the converted Rust code without any explanations.\n"""
    else:
...
def process_files():
    ...
    # 调试
    translation_order = ['test\\test-binary-heap.c']
    metadata['src\\binary-heap.c']['rust_items'] = """pub type BinaryHeapType = i32;
...
macro_rules! binary_heap_null { () => { None }; }

pub type BinaryHeapValue<T> = Option<T>;

pub type BinaryHeapCompareFunc<T> = fn(value1: &BinaryHeapValue<T>, value2: &BinaryHeapValue<T>) -> i32;

pub type BinaryHeap<T> = _BinaryHeap<T>;


pub struct _BinaryHeap<T> {
    pub heap_type: BinaryHeapType,
    pub values: Vec<BinaryHeapValue<T>>,
    pub num_values: u32,
        ...
        for test_func in test_funcs:
            ...
            for depend_func, depend_file in depend_files_and_funcs:
                ...
                if translated_flags[depend_file][depend_func] != None:
                    logging.info(f"依赖文件{depend_file}的函数{depend_func}之前已经翻译，无需再次翻译")
            ...
            if test_success and not dynamic_errors:
                logging.info(f"测试文件{problem_path}的测试函数 {test_func} 测试成功")
                ...
                for depend_func, depend_file in depend_files_and_funcs:
                    if translated_flags[depend_file][depend_func] == None:
                        translated_flags[depend_file][depend_func] = False
                logging.info(f"测试文件{problem_path}的测试函数 {test_func} 测试失败")
    # Write back metadata to file
    with open('tool/c_metadata_direct_output.json', 'w', encoding='utf-8') as f:
